...
function showAllCounts() {
    showCountInBoard();
    showCount('countInProgress','split','in_progress')
    showCount('countAwaitFeedback','split','awaiting_feedback')
    showCount('countToDo','split','to_do')
    showCount('countDone','split','done')
    showCountUrgent();
    showDlDate();
}
...
function showCount(elementID,param1,splintID){
    let countInXXX = document.getElementById(elementID);
    let count = 0;
    for (let i = 0; i < allTasks.length; i++) {
        console.log(allTasks[i][param1])
        if (allTasks[i][param1] === splintID) {
            count++;
        }
    }
    return countInXXX.innerHTML = count;
}
